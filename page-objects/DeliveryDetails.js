...
export class DeliveryDetails{
    constructor(page) {
        ...
        this.continutToPaymentButton = page.getByRole('button', { name: 'Continue to payment' })
    }
    ...
    saveDetails = async () => {
        const addressCountBeforeSaving = await this.savedAddressContainer.count() //рахуємо яка кількість контейнерів є. зберігаємо в константу
        await this.saveAddressButton.waitFor()
        await this.saveAddressButton.click()
        await expect(this.savedAddressContainer).toHaveCount(addressCountBeforeSaving + 1) //провіряємо що кількість контейнерів збільшлося на +1

        await this.savedAddressFirstName.first().waitFor() //чекаємо на перший елемент з контейнеру
        expect(await this.savedAddressFirstName.first().innerText()).toBe(await this.firstNameInput.inputValue()) //очікуємо що вміст першого елемент з контейнеру буде дорівнювати значенню яке ми ввели в поле до цього
    ...
    continueToPayment = async () => {
        await this.continutToPaymentButton.waitFor()
        await this.continutToPaymentButton.click()
